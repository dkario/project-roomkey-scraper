...
def num_with_commas_to_int(num):
    ...
    with pdfplumber.open(filename) as pdf:
        ...
        for i in range(len(words)):
            if words[i] == "Overall" and words[i + 1] == "Totals:":
                rooms_under_contract = num_with_commas_to_int(words[i + 2])
                rooms_operational = rooms_under_contract
                rooms_occupied = num_with_commas_to_int(words[i + 3])
                people_in_rooms = num_with_commas_to_int(words[i + 4])

                daily_totals = DailyTotals(
                    rooms_under_contract,
                    rooms_operational,
                    rooms_occupied,
                    people_in_rooms,
                )
